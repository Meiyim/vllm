...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def execute_model(
        ...
        if self.supports_mm_inputs and get_pp_group().is_first_rank:
            ...
            if num_input_tokens < 100:
                topk_ids = self.input_logprob_ids[:num_input_tokens,1:] # [B,20]
                orig_shape = topk_ids.shape
                embbed = emb(topk_ids.contiguous().view(-1))
                embbed = embbed.view(orig_shape+(embbed.shape[-1],)) #[B,20,h]
                logprob = self.input_logprobs[:num_input_tokens,1:] #[B,20]
        ...
        for req_idx, sampled_ids in enumerate(valid_sampled_token_ids):
            ...
            lpsum= logprobs_tensors.logprobs[req_idx].exp().sum()
